...
import { editAppointment, scheduleAppointment } from "@/app/backend/actions/appointmentActions";
import { toast } from "sonner";
import JSConfetti from "js-confetti";
import { newAppointmentSelectors, useNewAppointmentStore } from "@/app/store/newAppointmentStore";
import { useMemo } from "react";
import { Constants } from "@/app/utils/Constants";

export function useAppointment(onAppointmentSuccess: () => void) {
  const isRescheduleMode = useNewAppointmentStore(newAppointmentSelectors.isRescheduleMode);
  const appointmentId = useNewAppointmentStore(newAppointmentSelectors.appointmentId);

  const { mutate: mutateNewAppointment, isPending: isPendingNewAppointment } = useMutation({
    ...
    mutationFn: (newAppointmentData: IBaseNewAppointmentInfo) => {
      ...
      if (isRescheduleMode && appointmentId) {
        return editAppointment(appointmentId, appointmentData, Constants.TIMESLOTS.DEFAULT_DAYS_FORWARD);
      } else {
        return scheduleAppointment(appointmentData, Constants.TIMESLOTS.DEFAULT_DAYS_FORWARD);
      }
    },
    onError: () => {
