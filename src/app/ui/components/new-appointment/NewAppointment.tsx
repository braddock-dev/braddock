...
export default function NewAppointment(props: INewAppointmentProps) {
  ...
  const isAppointmentValid = useNewAppointmentStore(newAppointmentSelectors.isAppointmentValidWithoutCustomer);
  ...
  useEffect(() => {
    setEmployeeId(selectedOperator?.id);
  }, [selectedOperator]);
  ...
  const { mutate: newAppointmentMutation, isPending } = useMutation({
    ...
    mutationFn: (data: { appointmentData: INewAppointmentRequestData }) =>
      scheduleAppointment(data.appointmentData, Constants.TIMESLOTS.DEFAULT_DAYS_FORWARD),
    onError: () => {
